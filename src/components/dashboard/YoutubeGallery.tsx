...
export default function YoutubeGallery({ urls }: YoutubeGalleryProps) {
	...
	return (
		<div className="mt-12 grid max-w-2xl mx-4 grid-cols-1 gap-x-8 gap-y-16 sm:grid-cols-2 lg:mx-0 lg:max-w-none lg:grid-cols-4">
			{videos.map((video, key) => (
				<div key={key}>
					<a href={video.url} target="_blank">
						<img className="aspect-[7/4] w-full rounded-xl object-cover" src={video.thumbnailUrl} alt={video.title} />
						<h3 className="mt-6 text-lg font-semibold leading-6 tracking-tight text-gray-900 dark:text-slate-300">{video.title}</h3>
						<p className="text-base leading-7 text-slate-400">Youtube video</p>
					</a>
				</div>
			))}
		</div>
